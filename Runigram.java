...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for(int i =0; i < image.length; i++) {
			for(int j =0; j < image[i].length; j++){
				new_image[i][j] = image[i][image[0].length - 1 -j];
			}
		}
		return new_image;
	}
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for(int i =0; i < image.length; i++) {
			for(int j =0; j < image[i].length; j++){
				new_image[i][j] = image[image.length -1 -i][j];
			}
		}
		return new_image;
	}
	...
	public static Color luminance(Color pixel) {
		//// Replace the following statement with your codef
		int r = pixel.getRed();
		int g = pixel.getGreen();
		int b = pixel.getBlue();
		int lum = (int) (r * 0.299 + g * 0.587 + b * 0.114); 
		Color new_color = new Color(lum, lum, lum);
		return new_color;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		//// Replace the following statement with your code
		Color[][] new_image = new Color[height][width];
		for(int i = 0; i < height; i++ ) {
			for(int j = 0; j < width; j++)
			{
				new_image[i][j] = image[(i * image.length) /height][(j * image[0].length) / width];
			}
		}
		return new_image;
	}
	...
	public static Color[][] blend(Color[][] image1, Color[][] image2, double alpha) {
		...
		for(int i = 0; i < image1.length; i++ ) {
			for(int j = 0; j < image1[0].length; j++){
				image[i][j] = blend(image1[i][j], image2[i][j], alpha);	
			}
		}
		return image;
	}
